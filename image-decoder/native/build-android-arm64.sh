...
mkdir -p ./build-android-arm64/fakeroot
...
cat << EOF > "android-arm64-cross_file.txt"
[host_machine]
system = 'android'
cpu_family = 'aarch64'
cpu = 'aarch64'
...
[binaries]
c = '$TOOLCHAIN_PATH/bin/aarch64-linux-android26-clang'
cpp = '$TOOLCHAIN_PATH/bin/aarch64-linux-android26-clang++'
...
cmake -G Ninja \
    ...
    -DMESON_CROSS_FILE="$(readlink -f ./android-arm64-cross_file.txt)" \
...
cp "${TOOLCHAIN_PATH}/lib/clang/18/lib/linux/aarch64/libomp.so" ./fakeroot/lib
