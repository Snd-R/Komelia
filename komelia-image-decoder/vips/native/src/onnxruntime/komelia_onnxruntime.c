...
int enable_dml(JNIEnv *env, int device_id, OrtSessionOptions *options) {
    ...
    ORT_INT_STATUS_THROW(env, g_ort->GetExecutionProviderApi("DML", ORT_API_VERSION, (const void **) &ortDmlApi));
    ...
    ORT_INT_STATUS_THROW(env, ortDmlApi->SessionOptionsAppendExecutionProvider_DML(options, device_id));
    return 0;
}
#endif
...
int init_onnxruntime_session(JNIEnv *env) {
    ...
    onnx_status = g_ort->SetSessionGraphOptimizationLevel(current_session.session_options, ORT_ENABLE_BASIC);
    if (onnx_status != NULL) {
        throw_jvm_ort_exception(env, g_ort->GetErrorMessage(onnx_status));
        g_ort->ReleaseStatus(onnx_status);
        return -1;
    ...
    onnx_status = g_ort->CreateSession(ort_env, wide_model_path,
                                       current_session.session_options, &current_session.session);
    ...
    onnx_status = g_ort->CreateCpuMemoryInfo(OrtArenaAllocator, OrtMemTypeDefault, &current_session.memory_info);
    if (onnx_status != NULL) {
        throw_jvm_ort_exception(env, g_ort->GetErrorMessage(onnx_status));
        g_ort->ReleaseStatus(onnx_status);
        return -1;
...
OrtStatus *create_tensor_f32(
    ...
    *tensor_data = (float *) malloc(tensor_data_len);
    ...
    return g_ort->CreateTensorWithDataAsOrtValue(memory_info,
...
VipsImage *run_inference(JNIEnv *env,
    ...
    ORT_RELEASE_ON_ERROR(env, inference_data, g_ort->GetTensorElementType(input_tensor_info, &input_element_type));
    ...
    size_t dim_length;
    ORT_RELEASE_ON_ERROR(env, inference_data, g_ort->GetDimensionsCount(inference_data.out_tensor_info, &dim_length));
...
Java_snd_komelia_image_OnnxRuntimeUpscaler_init(JNIEnv *env, jclass this, jstring provider) {
...
JNIEXPORT void JNICALL
Java_snd_komelia_image_OnnxRuntimeUpscaler_setTileSize(JNIEnv *env, jclass this, jint tile_size) {
    current_tile_size = tile_size;
}

JNIEXPORT void JNICALL
Java_snd_komelia_image_OnnxRuntimeUpscaler_setModelPath(JNIEnv *env, jclass this, jstring model_path) {
...
JNIEXPORT void JNICALL
Java_snd_komelia_image_OnnxRuntimeUpscaler_setDeviceId(JNIEnv *env, jclass this, jint device_id) {
    pthread_mutex_lock(&session_mutex);
    release_session(&current_session);
...
JNIEXPORT void JNICALL Java_snd_komelia_image_OnnxRuntimeUpscaler_closeCurrentSession(JNIEnv *env, jclass this) {
    pthread_mutex_lock(&session_mutex);
    release_session(&current_session);
    pthread_mutex_unlock(&session_mutex);
}
...
VipsImage *tiled_inference(JNIEnv *env, VipsImage *input_image) {
    ...
    while (y_taken != image_height) {
        ...
        while (x_taken != image_width) {
            ...
            unformatted_image = vips_image_new_from_memory(
                    region_data,
                    region_size,
                    region_rect.width,
                    region_rect.height,
            ...
            vips_copy(unformatted_image, &formatted_region_image,
            ...
            if (x_taken > image_width) x_taken = image_width;
            ++tile_count;
        }

        y_taken = y_taken + tile_size;
        if (y_taken > image_height) y_taken = image_height;
    }
    pthread_mutex_unlock(&session_mutex);
...
JNIEXPORT jobject JNICALL Java_snd_komelia_image_OnnxRuntimeUpscaler_upscale(
        JNIEnv *env,
        jclass this,
        jobject vips_image
) {
