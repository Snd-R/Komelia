...
JNIEXPORT jobject JNICALL Java_snd_komelia_image_AndroidBitmap_createHardwareBuffer(
    ...
    int lock_error = AHardwareBuffer_lock(
        hardware_buffer,
    ...
    if (lock_error) {
        komelia_throw_jvm_vips_exception_message(
            env,
            "Could not acquire created hardware hardware_buffer"
        );
    ...
    if (created_desc.stride == image_width) {
        ...
        for (int y = 0; y < image_height; ++y) {
            memcpy(
                write_buffer + (created_desc.stride * y * 4),
...
JNIEXPORT jobject JNICALL Java_snd_komelia_image_AndroidBitmap_createSoftwareBitmap(
    ...
    jfieldID config_enum_field = (*env)->GetStaticFieldID(
        env,
        config_enum_class,
        "ARGB_8888",
    ...
    jobject jvm_bitmap = (*env)->CallStaticObjectMethod(
        env,
        bitmap_class,
        create_method,
        image_width,
