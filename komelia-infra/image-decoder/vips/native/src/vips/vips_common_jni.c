...
void komelia_throw_jvm_vips_exception_message(
    JNIEnv *env,
    const char *message
) {
...
jobject get_jvm_enum_type(
    ...
    jfieldID enum_field =
        (*env)
            ->GetStaticFieldID(env, enum_class, enum_field_name, "Lsnd/komelia/image/ImageFormat;");
    return (*env)->GetStaticObjectField(env, enum_class, enum_field);
}

int transform_to_supported_format(
    JNIEnv *env,
    VipsImage *in,
...
VipsImage *komelia_from_jvm_handle(
    JNIEnv *env,
    jobject jvm_image
) {
...
jobject komelia_to_jvm_handle(
    ...
    return (*env)->NewObject(
        ...
        jvm_delay_array,
        jvm_type_enum,
        (int64_t)external_source_buffer,
        (int64_t)transformed
    );
}
